...
#inputpath = os.getcwd() + "/nt-files/bp.nt"
#x = glob.glob("%s*.nt" %inputpath)

current_file = os.getcwd() + "/ISWC.nt"
#Path for the imput file containing triples about ISWC.

outputpath="ISWC-2015.nt" #Output file with triples related to only ISWC Conferences
...
qres = g.query(
                   """SELECT ?a ?b ?c
                    WHERE {
                    ?a ?b ?c .
                    ?a <http://dblp.org/rdf/schema-2017-04-18#yearOfPublication> "2015" .
